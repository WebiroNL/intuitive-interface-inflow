  ...
  return (
    <div className="flex items-center gap-2">
      <Select value={String(month)} onValueChange={(v) => onChange(Number(v), year)}>
        <SelectTrigger className="w-[140px] h-9 text-[13px] bg-white text-foreground border-border">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {MONTHS.map((m, i) => (
      ...
      <Select value={String(year)} onValueChange={(v) => onChange(month, Number(v))}>
        <SelectTrigger className="w-[100px] h-9 text-[13px] bg-white text-foreground border-border">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {years.map((y) => (
