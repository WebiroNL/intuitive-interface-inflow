...
function GlassFilter() {
  return (
    <svg className="absolute inset-0 size-0 pointer-events-none" aria-hidden="true">
      <defs>
        <filter
...
function LiquidButton({
  ...
  return (
    <>
      <div className="relative group/liquid inline-flex">
        {/* Glass background layer */}
        <div
          className="glass-bg pointer-events-none absolute inset-0 rounded-xl border border-white/20 bg-white/10 shadow-lg backdrop-blur-md dark:border-white/10 dark:bg-white/5"
          style={{ filter: "url(#liquid-glass-filter)" }}
        />

        {/* Shimmer / hover highlight */}
        <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-xl">
          <div className="absolute inset-0 translate-x-[-100%] bg-gradient-to-r from-transparent via-white/10 to-transparent transition-transform duration-700 group-hover/liquid:translate-x-[100%]" />
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {buttonText}
        {isHovered && !isPressed && !isTouchDevice && (
          <div className="pointer-events-none absolute inset-0 rounded-md bg-white/5" />
        )}
      </button>
    </div>
  )
