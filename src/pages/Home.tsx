...
import { MacbookShowcase, type ShowcaseItem } from "@/components/MacbookShowcase";
...
const WebsiteMockup = ({ accent }: { accent: "primary" | "accent" }) => {
  ...
  useLayoutEffect(() => {
    const check = () => {
      ...
      if (!c || !b) return;
      // Measure natural height of the bottom row
      const prevDisplay = b.style.display;
      b.style.display = "grid";
      const bRect = b.getBoundingClientRect();
      const cRect = c.getBoundingClientRect();
      const bTop = bRect.top - cRect.top;
      const fits = bTop + bRect.height <= cRect.height + 0.5;
  ...
  return (
    <div ref={containerRef} className="relative w-full rounded-xl overflow-hidden border border-border/60 shadow-lg bg-card" style={{ aspectRatio: "16/10" }}>
      ...
      <div className={`absolute inset-0 top-9 flex flex-col ${accent === "primary" ? "bg-gradient-to-br from-primary/5 via-background to-primary/10" : "bg-gradient-to-br from-accent/5 via-background to-accent/10"}`}>
        ...
        <div
          ref={bottomRef}
          className="mt-auto px-4 pb-3 pt-2 grid grid-cols-3 gap-2 shrink-0"
      ...
      <div className="absolute inset-0 top-9 bg-gradient-to-br from-accent/5 via-background to-accent/10 p-3">
        ...
        <div className="rounded-lg border border-border/30 bg-background/60 p-2">
          ...
          <div className="flex items-end gap-1 h-8">
            {bars.map((h, i) => {
              const low = Math.max(15, h - 30);
              const high = Math.min(100, h + 10);
              return (
                <motion.div
                  key={i}
                  animate={{ height: [`${low}%`, `${high}%`, `${low}%`] }}
...
const showcase = [
  ...
  { title: "CKN Legal", cat: "Juridisch", url: "https://www.cknlegal.com", services: ["Branding", "Website"], desc: "Professionele huisstijl en website die vertrouwen uitstraalt voor een groeiend advocatenkantoor.", tint: "215,55%,40%", previewImageUrl: "/portfolio/cknlegal.webp" },
